...
function LogoBadge({ name, icon }: { name: string; icon: ReactNode }) {
  return (
    <motion.div
      whileHover={{ y: -2 }}
      className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-foreground/[0.08] bg-foreground/[0.02] hover:bg-foreground/[0.04] transition-colors"
    >
...
function LogoRow({ items }: { items: { name: string; icon: ReactNode }[] }) {
  return (
...
export function ProviderLogos() {
  return (
    <div className="mt-10 space-y-5">
      <div>
        <p className="text-[11px] uppercase tracking-wider text-foreground/40 font-medium mb-3">
          Providers
        </p>
        <LogoRow items={providers} />
      </div>
